...
rock1_image = cv2.imread('rock_on_2.png', cv2.IMREAD_UNCHANGED)
rock2_image = cv2.imread('rock_on_1.png', cv2.IMREAD_UNCHANGED)
...
def overlay_image(frame, img, x, y, size):
    ...
    if y + h > frame.shape[0] or x + w > frame.shape[1]:
        return  # No superponer si la imagen se sale del frame
    ...
    if img.shape[2] == 4:
        ...
        # Crear una máscara del canal alfa
        mask = a  # El canal alfa es la máscara
        # Superponer usando la máscara
        for c in range(3):  # Para cada canal BGR
            frame[y:y+h, x:x+w, c] = frame[y:y+h, x:x+w, c] * (1 - mask / 255.0) + img[:, :, c] * (mask / 255.0)
    else:
        # Superponer sin canal alfa
...
while True:
    ...
    if L_fingers == [0, 1, 0, 0, 1]:
        ...
        if L_handLandmarks:
            x1=L_handLandmarks[0][1]
            y1=L_handLandmarks[0][2]
            x2 = L_handLandmarks[8][1]
            y2 = L_handLandmarks[8][2]
            size = int(((x1 - x2)**2 + (y1 - y2)**2)**0.5)
            overlay_image(frame, rock1_image, x1 - 70, y1 - 180, size - 20)
    ...
    if R_fingers == [0, 1, 0, 0, 1]:
        ...
        if R_handLandmarks:
            x1 = R_handLandmarks[0][1]
            y1 = R_handLandmarks[0][2]
            x2 = R_handLandmarks[8][1]
            y2 = R_handLandmarks[8][2]
            size = int(((x1 - x2)**2 + (y1 - y2)**2)**0.5)
            overlay_image(frame, rock2_image, x1 - 50, y1 - 180, size - 20)
    ...
    if(R_fingers==[1, 0, 1, 0, 0]):
        ...
        cv2.circle(frame, (x1,y1), radius=R, color=(255, 255, 255), thickness=-1)
        cv2.putText(frame, 'Right hand: F#@k you', (100, 400),font, fontScale=font_scale, color=(255, 255, 255), thickness=2)   
